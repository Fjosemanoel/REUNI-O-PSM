...
function normalizeDailyOrderArchive(value){
  if(!value||typeof value!=='object'||Array.isArray(value))return{};
  const normalized={};
  Object.entries(value).forEach(([weekKey,days])=>{
    const week=Number(weekKey);if(!Number.isInteger(week)||week<1||week>52||!days||typeof days!=='object'||Array.isArray(days))return;
    const cleanDays={};
    Object.entries(days).forEach(([date,orders])=>{
      if(!/^\d{4}-\d{2}-\d{2}$/.test(date)||!Array.isArray(orders))return;
      const seen=new Set();
      const cleanOrders=orders.map(order=>{
        const normalizedOrder=normalizeOrderRecord({...order,dailyArchived:true,dailyArchiveWeek:week,dailyArchiveDate:date});
        normalizedOrder.dailyArchiveKey=normalize(order?.dailyArchiveKey)||upper(normalizedOrder.ordem)||normalize(normalizedOrder.id);
        return normalizedOrder;
      }).filter(order=>{
        if(!order.dailyArchiveKey||seen.has(order.dailyArchiveKey)||!isDailyPlanOrder(order))return false;
        seen.add(order.dailyArchiveKey);return true;
      });
      if(cleanOrders.length)cleanDays[date]=cleanOrders;
    });
    if(Object.keys(cleanDays).length)normalized[String(week)]=cleanDays;
  });
  return normalized;
}
...
function getProjectData(options={}){
  syncActiveFilterBank();
  const project={orders:state.orders,systematicCatalog:state.systematicCatalog,systematicCatalogMeta:state.systematicCatalogMeta,capacity:state.capacity,capacityChartAreas:state.capacityChartAreas,capacityAreaSelectionExplicit:state.capacityAreaSelectionExplicit,capacityConsumptionOffice:state.capacityConsumptionOffice,dailySelectedWeek:state.dailySelectedWeek,dailySelectedDay:state.dailySelectedDay,dailySelectedPromanPlants:state.dailySelectedPromanPlants,dailySelectedOffices:state.dailySelectedOffices,dailyObservations:state.dailyObservations,dailyOrderArchive:state.dailyOrderArchive,history:state.history,qppBoard:state.qppBoard,qppSelectedWeeks:state.qppSelectedWeeks,meetings:state.meetings,filtersByView:state.filtersByView};
...
function captureSharedPresentationSettings(){
  return{
    ...
    capacityAreaSelectionExplicit:state.capacityAreaSelectionExplicit,
...
function normalizeSharedPresentationSettings(value={}){
  ...
  return{
    ...
    capacityAreaSelectionExplicit:value.capacityAreaSelectionExplicit===true||(Array.isArray(value.capacityChartAreas)&&value.capacityChartAreas.length>0),
...
function applySharedPresentationSettings(value){
  ...
  state.capacityAreaSelectionExplicit=settings.capacityAreaSelectionExplicit;
...
function getSharedProjectData(){
  ...
  return{
    ...
    dailyOrderArchive:state.dailyOrderArchive,
...
function applySharedProjectData(payload,metadata={}){
  ...
  state.dailyOrderArchive=normalizeDailyOrderArchive(payload.dailyOrderArchive);
...
async function importProject(file){
  ...
  state.capacityAreaSelectionExplicit=payload.capacityAreaSelectionExplicit===true||state.capacityChartAreas.length>0;
...
function load(){
  try{
    const s=JSON.parse(localStorage.getItem(STORAGE_KEY));if(!s)return;
    state.orders=(s.orders||[]).map(normalizeOrderRecord);
    state.systematicCatalog=(s.systematicCatalog||[]).map(item=>({...item,tipoOrdem:'SISTEMÁTICA'}));
    state.systematicCatalogMeta=s.systematicCatalogMeta||{fileName:'',importedAt:'',total:state.systematicCatalog.length};
    state.capacity=s.capacity||[];
    state.capacityChartAreas=Array.isArray(s.capacityChartAreas)?s.capacityChartAreas.map(upper):[];
    state.capacityAreaSelectionExplicit=s.capacityAreaSelectionExplicit===true||state.capacityChartAreas.length>0;
    state.capacityConsumptionOffice=upper(s.capacityConsumptionOffice);
    state.dailySelectedWeek=Number(s.dailySelectedWeek)||0;
    state.dailySelectedDay=['0','1','2','3','4'].includes(String(s.dailySelectedDay))?String(s.dailySelectedDay):'';
    state.dailySelectedPromanPlants=Array.isArray(s.dailySelectedPromanPlants)?s.dailySelectedPromanPlants.filter(value=>['britagem','fabrica'].includes(value)):[];
    state.dailySelectedOffices=Array.isArray(s.dailySelectedOffices)?s.dailySelectedOffices.map(upper).filter(Boolean):[];
    state.dailyObservations=normalizeDailyObservations(s.dailyObservations);
    state.dailyOrderArchive=normalizeDailyOrderArchive(s.dailyOrderArchive);
    state.history=s.history||[];
    state.qppBoard=s.qppBoard||[];
    state.qppSelectedWeeks=normalizeQppWeekSelection(s.qppSelectedWeeks);
    state.meetings=s.meetings||{};
    state.filtersByView={dashboard:cloneFilterState(s.filtersByView?.dashboard),ordens:cloneFilterState(s.filtersByView?.ordens)};
...
function allCapacityAreaValues(){return[...new Set(state.capacity.map(item=>upper(item.area)).filter(Boolean))];}
function selectedCapacityAreaValues(){return state.capacityAreaSelectionExplicit?[...new Set(state.capacityChartAreas.map(upper).filter(Boolean))]:allCapacityAreaValues();}
function capacityAreaIsVisible(value){return!state.capacityAreaSelectionExplicit||state.capacityChartAreas.map(upper).includes(upper(value));}
function renderCapacityConsumption(){
  ...
  const selectedAreas=selectedCapacityAreaValues();
  const areaAllowed=area=>capacityAreaIsVisible(area);
...
function renderCharts(){const rows=getFiltered(),qpp=rows.filter(o=>qppValue(o)==='QPP'),c=vcTheme();const areaHH=groupSum(qpp,'area'),areaCost=groupSum(rows,'area','custo'),workshopHH=groupSum(qpp,'oficina');const selectedAreas=selectedCapacityAreaValues();const areaAllowed=value=>capacityAreaIsVisible(value);const capacityQpp=qpp.filter(order=>areaAllowed(order.area));const capacity=groupSum(state.capacity.filter(item=>areaAllowed(item.area)&&(!state.filters.area.length||state.filters.area.includes(item.area))&&(!state.filters.oficina.length||state.filters.oficina.includes(item.oficina))),'area');const capacityQppByArea=groupSum(capacityQpp,'area');const allAreas=[...new Set([...Object.keys(capacity),...Object.keys(capacityQppByArea)])];const capacityLabel=$('#capacityChartFilterLabel');if(capacityLabel)capacityLabel.textContent=state.capacityAreaSelectionExplicit?(selectedAreas.length?`Áreas: ${selectedAreas.join(', ')}`:'Nenhuma área selecionada'):'Todas as áreas';
...
function clearSystematicData(){
  captureSelectedDailyWeekOrders();
  clearTimeout(dailyArchiveSaveTimer);dailyArchiveSaveTimer=0;
...
function dailyArchiveOrderKey(order){return normalize(order?.dailyArchiveKey)||upper(order?.ordem)||normalize(order?.id);}
function dailyArchiveSnapshot(order,week,date,existing=null){
  const completed=isDailyCompleted(order);
  return normalizeOrderRecord({
    id:normalize(existing?.id)||normalize(order?.id)||uid(),
    ordem:normalize(order?.ordem),descricao:normalize(order?.descricao),area:normalize(order?.area),oficina:upper(order?.oficina),
    equipamento:normalize(order?.equipamento),grupo:normalize(order?.grupo),criticidade:normalize(order?.criticidade),prioridade:normalize(order?.prioridade),
    duracao:num(order?.duracao),maoObra:num(order?.maoObra),hh:num(order?.hh),custo:num(order?.custo),
    qpp:qppValue(order),tipoOrdem:orderTypeValue(order),realizado:completed,
    completionDate:completed?(normalize(order?.completionDate)||normalize(existing?.completionDate)):'',
    source:normalize(order?.source)||normalize(existing?.source)||'programacao',
    dailyArchived:true,dailyArchiveWeek:Number(week),dailyArchiveDate:date,
    dailyArchiveKey:dailyArchiveOrderKey(order)||dailyArchiveOrderKey(existing),
    dailySourceId:normalize(order?.id)||normalize(existing?.dailySourceId)
  });
}
let dailyArchiveSaveTimer=0;
function scheduleDailyArchiveSave(){
  clearTimeout(dailyArchiveSaveTimer);
  dailyArchiveSaveTimer=setTimeout(()=>{dailyArchiveSaveTimer=0;save();},250);
}
function captureDailyWeekOrders(week,eligible){
  const weekKey=String(Number(week.week));
  const stored=state.dailyOrderArchive[weekKey]&&typeof state.dailyOrderArchive[weekKey]==='object'?state.dailyOrderArchive[weekKey]:{};
  const nextDays={...stored};
  let changed=false;
  week.days.forEach(day=>{
    const date=normalize(day.date);if(!date)return;
    const areas=dayMapAreas(day);
    const existing=Array.isArray(stored[date])?stored[date]:[];
    const byKey=new Map(existing.map((order,index)=>[dailyArchiveOrderKey(order),index]));
    const next=[...existing];
    eligible
      .filter(order=>areas.some(area=>orderMatchesMapArea(order,area)))
      .filter(order=>!isDailyCompleted(order)||normalize(order.completionDate)===date)
      .forEach(order=>{
        const key=dailyArchiveOrderKey(order);if(!key)return;
        const index=byKey.get(key),previous=index===undefined?null:next[index];
        const snapshot=dailyArchiveSnapshot(order,week.week,date,previous);
        if(!previous){byKey.set(key,next.length);next.push(snapshot);changed=true;}
        else if(JSON.stringify(previous)!==JSON.stringify(snapshot)){next[index]=snapshot;changed=true;}
      });
    if(next.length)nextDays[date]=next;
  });
  if(changed||state.dailyOrderArchive[weekKey]!==stored){state.dailyOrderArchive[weekKey]=nextDays;scheduleDailyArchiveSave();}
  return nextDays;
}
function captureSelectedDailyWeekOrders(){
  ensureQppBoard();
  const selected=Number(state.dailySelectedWeek)||automaticPlanningWeek();
  const week=state.qppBoard.find(item=>Number(item.week)===selected);
  if(week)captureDailyWeekOrders(week,state.orders.filter(isDailyPlanOrder));
}
function updateDailyArchivedCompletion(week,archiveKey,completed,completionDate){
  const days=state.dailyOrderArchive[String(Number(week))];if(!days||!archiveKey)return false;
  let changed=false;
  Object.values(days).forEach(orders=>orders.forEach(order=>{
    if(dailyArchiveOrderKey(order)!==archiveKey)return;
    order.realizado=Boolean(completed);order.completionDate=completed?completionDate:'';changed=true;
  }));
  return changed;
}
function findDailyArchivedOrder(week,archiveKey){
  const days=state.dailyOrderArchive[String(Number(week))]||{};
  for(const orders of Object.values(days)){const found=orders.find(order=>dailyArchiveOrderKey(order)===archiveKey);if(found)return found;}
  return null;
}
function dailyArchivedCount(week){
  const days=state.dailyOrderArchive[String(Number(week))]||{};
  return Object.values(days).reduce((total,orders)=>total+(Array.isArray(orders)?orders.length:0),0);
}
function dailyOrdersForDay(day,archivedOrders=[],promanActivities=[]){
  const belongsToDay=order=>!isDailyCompleted(order)||normalize(order.completionDate)===normalize(day.date);
  const psmOrders=archivedOrders.filter(belongsToDay);
...
function dailyOrderCard(order,day){
  ...
  return `<article class="daily-order">
    <div class="daily-order-heading"><strong>${escapeHtml(order.ordem)}</strong><span class="daily-order-controls">${badges}<label class="daily-checkin ${completed?'is-checked':''}" title="${completed?'Remover check-in':'Marcar atividade como realizada'}"><input type="checkbox" data-daily-checkin-id="${escapeHtml(order.id)}" data-daily-day="${escapeHtml(day?.date||'')}" ${order.dailyArchived?`data-daily-archive-key="${escapeHtml(dailyArchiveOrderKey(order))}" data-daily-source-id="${escapeHtml(order.dailySourceId||order.id)}"`:''} ${order.isProman?`data-proman-record-id="${escapeHtml(order.promanRecordId)}" data-proman-plant="${escapeHtml(order.promanPlant)}" data-proman-os="${escapeHtml(order.ordem)}" data-proman-what="${escapeHtml(order.descricao)}"`:''} ${completed?'checked':''} aria-label="${completed?'Remover check-in da':'Marcar como realizada a'} ordem ${escapeHtml(order.ordem)}"></label></span></div>
    <p>${escapeHtml(order.descricao||'Sem descrição')}</p>
    <small class="daily-order-meta">${escapeHtml(meta)}</small>
    ${detail}
    <label class="daily-observation"><span>Observação do dia</span><textarea rows="2" data-daily-observation-key="${escapeHtml(observationKey)}" data-daily-observation-order-id="${escapeHtml(order.id)}" data-daily-source-id="${escapeHtml(order.dailySourceId||order.id)}" placeholder="Escreva uma observação..." aria-label="Observação diária da ordem ${escapeHtml(order.ordem)}">${escapeHtml(dailyObservation)}</textarea></label>
  </article>`;
}
...
function persistDailyObservation(target){
  ...
  const order=state.orders.find(item=>item.id===(target.dataset.dailySourceId||target.dataset.dailyObservationOrderId));
...
function renderDailyPlan(){
  ...
  const archivedWeekOrders=captureDailyWeekOrders(week,eligible);
  ...
  const ordersByDay=new Map(indexedDays.map(({day})=>[day.date,dailyOrdersForDay(day,archivedWeekOrders[day.date]||[],promanActivities)]));
  const clearButton=$('#btnClearDailyArchive');
  if(clearButton){const archivedTotal=dailyArchivedCount(week.week);clearButton.textContent=`LIMPAR FIXADOS${archivedTotal?` (${archivedTotal})`:''}`;clearButton.disabled=!archivedTotal;}
...
function wireRequestedImprovements(){
  ...
  $('#btnClearDailyArchive')?.addEventListener('click',()=>{
    const week=Number(state.dailySelectedWeek)||automaticPlanningWeek(),total=dailyArchivedCount(week);
    if(!total){toast('Não há atividades fixadas nesta semana');return;}
    if(!confirm(`Limpar ${total} atividade(s) fixada(s) da semana ${String(week).padStart(2,'0')}? As ordens que ainda estiverem no backlog continuarão aparecendo normalmente.`))return;
    delete state.dailyOrderArchive[String(week)];
    state.dailyObservations=Object.fromEntries(Object.entries(state.dailyObservations).filter(([key])=>!key.startsWith(`${week}:`)));
    clearTimeout(dailyArchiveSaveTimer);dailyArchiveSaveTimer=0;
    save();renderDailyPlan();toast('Atividades fixadas da semana removidas');
  });
  ...
  $('#dailyPlanGrid')?.addEventListener('change',event=>{
    ...
    if(checkinId){
      if(event.target.dataset.promanRecordId!==undefined){
        let changed=false;
      ...
      const archiveKey=event.target.dataset.dailyArchiveKey||'';
      const sourceId=event.target.dataset.dailySourceId||checkinId;
      const order=state.orders.find(item=>item.id===sourceId)||state.orders.find(item=>item.id===checkinId);
      const archived=findDailyArchivedOrder(state.dailySelectedWeek,archiveKey);
      if(!order&&!archived){event.target.checked=!event.target.checked;toast('Não foi possível localizar esta atividade na Programação diária.');renderDailyPlan();return;}
      const completed=Boolean(event.target.checked),completionDate=completed?(event.target.dataset.dailyDay||new Date().toISOString().slice(0,10)):'';
      if(order){order.realizado=completed;order.completionDate=completionDate;}
      updateDailyArchivedCompletion(state.dailySelectedWeek,archiveKey,completed,completionDate);
      clearTimeout(dailyArchiveSaveTimer);dailyArchiveSaveTimer=0;
      const reference=order||archived;
      log(completed?'Check-in realizado':'Check-in removido',`OS ${reference.ordem} ${completed?'movida para Realizado':'devolvida para sua coluna de origem'}.`);
      renderDailyPlan();
      toast(completed?'Atividade marcada como realizada':'Atividade devolvida para o planejamento');
      return;
    }
    const observationKey=event.target.dataset.dailyObservationKey;
    if(!observationKey)return;
  ...
  $('#capacityBody')?.addEventListener('change',event=>{
    ...
    const selected=new Set(state.capacityAreaSelectionExplicit?state.capacityChartAreas.map(upper):allCapacityAreaValues());
    if(event.target.checked)selected.add(upper(area));else selected.delete(upper(area));
    state.capacityAreaSelectionExplicit=true;
    state.capacityChartAreas=[...selected];save();renderCapacity();renderCharts();renderCapacityConsumption();
    toast(state.capacityChartAreas.length?'Áreas visíveis atualizadas':'Nenhuma área selecionada');
  });
...
function wire(){
  ...
  $('#btnClearOrders').onclick=()=>{
    ...
    if(confirm(`Tem certeza que deseja apagar todas as ${total} ordens? Esta ação não apaga o catálogo SISTEMÁTICAS, HH disponível, Quadro QPP nem atas.`)){
      captureSelectedDailyWeekOrders();
      clearTimeout(dailyArchiveSaveTimer);dailyArchiveSaveTimer=0;
      state.orders=[];state.page=1;state.filters=emptyFilterState();$('#globalSearch').value='';
      log('Banco de ordens limpo',`${total} ordens foram removidas. O catálogo SISTEMÁTICAS e a Programação diária já fixada foram preservados.`);
      save();render();toast('Banco de ordens limpo · Programação diária preservada');
    }
  };
