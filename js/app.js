...
function wireMeetings(){Object.values(MEETING_TARGETS).forEach(sel=>{const root=$(sel);if(!root)return;root.addEventListener('change',e=>updateMeetingField(e.target));root.addEventListener('input',e=>{if(e.target.matches('input,textarea')&&!e.target.matches('[data-meeting-field="participant-name"]'))updateMeetingField(e.target);});root.addEventListener('click',e=>{const addKind=e.target.dataset.addMeetingRow;if(addKind){if(isViewerMode())return;const sheet=e.target.closest('.meeting-sheet'),key=sheet?.dataset.meeting,meeting=state.meetings?.[key];if(meeting&&Array.isArray(meeting[addKind])){meeting[addKind].push({tag:'',what:'',who:'',when:'',status:'',comments:''});renderMeeting(key);save();toast('Nova linha adicionada à ata');}return;}if(e.target.dataset.printMeeting)printDocument(e.target.dataset.printMeeting);});});}
...
function wireRequestedImprovements(){
  ...
  window.addEventListener('psm:proman-changed',renderDailyPlan);
}
...
updatePlanningWeek();load();ensureQppBoard();ensureMeetings();if(!state.orders.length){state.orders=sampleOrders.map(o=>normalizeOrderRecord({...o,id:uid()}));state.capacity=sampleCapacity;log('Dados de demonstração','Amostra inicial carregada. Importe sua planilha para substituir a base.');}if(localStorage.getItem('psm-theme')==='light')document.documentElement.classList.add('light');wire();wireRequestedImprovements();wireAccessModes();render();
